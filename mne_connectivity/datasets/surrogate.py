...
def make_surrogate_resting_data(
    ...
    Returns
    -------
    surrogate_data : list of ~mne.EpochsArray or ~mne.time_frequency.EpochsSpectrum or ~mne.time_frequency.EpochsTFR
        The surrogate data for the null hypothesis with ``n_shuffles`` entries. Returned
        as a generator if ``return_generator=True``.
    ...
    _validate_type(
        data,
        (BaseEpochs, EpochsSpectrum, EpochsTFR),
        "data",
        (
    ...
    if not isinstance(data, BaseEpochs) and not np.iscomplexobj(data.get_data()):
        raise TypeError(
...
def make_surrogate_evoked_data(
    ...
    Returns
    -------
    surrogate_data : list of ~mne.EpochsArray or ~mne.time_frequency.EpochsTFR
        The surrogate data for the null hypothesis with ``n_shuffles`` entries. Returned
        as a generator if ``return_generator=True``.
    ...
    Surrogate data is generated by cutting the time series at a random point and
    reversing the cut portion, independently for each epoch and channel. This destroys
    the covariance of the data, such that connectivity estimates should reflect the null
    hypothesis of no genuine connectivity between signals (e.g., only interactions due
    to background noise) :footcite:`AruEtAl2015`.
    ...
    n_chans, n_times = np.asarray(data.get_data().shape)[[1, -1]]
    if n_chans == 1:
        raise ValueError("Data must contain more than one channel for shuffling.")
    if n_times == 1:
...
def _shuffle_coefficients(data, n_shuffles, rng_seed, data_form):
    ...
    if isinstance(data, BaseEpochs):
        ...
        defaults = dict(
            ...
            reject=data.reject,
            flat=data.flat,
            reject_tmin=data.reject_tmin,
            reject_tmax=data.reject_tmax,
            baseline=data.baseline,
            proj=data.proj,
            metadata=data.metadata,
            selection=data.selection,
            drop_log=data.drop_log,
            raw_sfreq=data.info["sfreq"],
        )
    else:
        defaults = dict(
            method=None, tmin=None, tmax=None, picks=None, proj=None, n_jobs=None
        )
        if isinstance(data, EpochsSpectrum):
            data_class = EpochsSpectrum
            defaults.update(dict(fmin=None, fmax=None, exclude=(), remove_dc=None))
        else:
            data_class = EpochsTFR
            defaults.update(dict(freqs=None, decim=None))
...
def _shuffle_coefficients_over_epochs(
    ...
    for _ in range(n_shuffles):
        ...
        # Package surrogate data for this shuffle (return as generator)
        if data_class == EpochsArray:
            yield data_class(data=surrogate_arr, **defaults)
        else:
            state["data"] = surrogate_arr
            yield data_class(inst=state, **defaults)
