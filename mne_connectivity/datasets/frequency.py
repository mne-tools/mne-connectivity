...
def make_signals_in_freq_bands(
    ...
    *,
    ...
    duration : float (default 2.0)
        ...
        .. versionchanged:: 1.0
           Replaced ``n_times`` as the way to specify epoch length.
    ...
    connection_delay : float (default 0.05)
        Connectivity delay between the seeds and targets, in seconds. If > 0, the target
        data is a delayed form of the seed data. If < 0, the seed data is a delayed form
        of the target data. The absolute value must be less than ``duration``.

        .. versionchanged:: 1.0
           Units changed from samples to seconds.
