...
def check_multivariate_indices(indices, n_chans=None):
    ...
    n_chans : int | None (default None)
        The number of channels in the data. Used when converting negative
        indices to positive indices. Cannot be ``None`` if negative indices are
        present.
    ...
    for inds in ([*indices[0], *indices[1]]):
        ...
        if any(idx < 0 for idx in inds):
            if n_chans is None:
                raise ValueError(
                    '`n_chans` must be given if there are negative values '
                    'in `indices`')
            if any(idx * -1 > n_chans for idx in inds[inds < 0]):
                raise ValueError(
                    'a channel index is not present in the data'
                )
            inds[inds < 0] = inds[inds < 0] % n_chans
