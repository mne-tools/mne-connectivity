...
2. Create a pull request to the `main` branch with the following changes:
    ...
    c. Run the `tools/generate_citation.py` script with the new version number as an argument (including micro version information, e.g., `0.8.0`) to update the information in `CITATION.cff` (and in turn the package citation in `doc/references.bib`). Note, this will use the current date for the release date field.
