...
# positive-valued, and is not bound below 1. MIM can be normalised to lie in
# the range :math:`[0, 1]` by dividing by :math:`N/2` (where :math:`N` is the
# number of channels in the seeds and targets). **MIM is not normalised by
# default in our implementation**. Without normalisation, MIM can be thought of
# as reflecting the total interaction between the seeds and targets, whereas
# normalised MIM reflects the total interaction *per channel*.
