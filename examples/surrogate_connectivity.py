...
#
# Such surrogate data can be easily generated in MNE-Connectivity using the
...
freqs = np.arange(4, 23, 2)  # take every other frequency to reduce compute time
tfr_kwargs = dict(
    freqs=freqs,
    n_cycles=freqs / 2.0,
    method="morlet",
    decim=3,  # decimate in time-domain to reduce compute time
    output="complex",
    n_jobs=1,
)
...
# contains a temporal structure that is consistent across epochs. Shuffling these epochs
# across channels (as is done in :func:`~mne_connectivity.make_surrogate_resting_data`)
# will fail to adequately disrupt the covariance structure.
#
...
#
# Although the alpha connectivity was still far above the baseline from the evoked
# surrogates, this will not always be the case. You can see how this risks false
# negative assessments that connectivity is not significantly different from baseline.
# Surrogate connectivity estimates from evoked data should therefore always be
