...
class MEGLDS(_MEGModel):
    ...
    def __init__(self, num_roi, timepts, A=None, roi_cov=None, mu0=None, roi_cov_0=None, 
        ...
        timepts, num_roi, _ = self.A.shape
    ...
    def add_subject(self, subject, subject_dir, epochs,labels, fwd, cov):
        ...
        scales = {'eeg_scale' : 1, 'mag_scale' : 1, 'grad_scale' : 1} #scale=1 has no effect
        fwd_src_snsr, fwd_roi_snsr, snsr_cov, epochs = \
                _scale_sensor_data(epochs, fwd, cov, roi_to_src, **scales)
                
        # which_roi = roi_to_src.which_roi # array of len(sources); val = ROI of source
        # data = epochs._data
        # data = np.swapaxes(data,-1,-2)
        # subjectdata = [(data, fwd_roi_snsr, fwd_src_snsr, snsr_cov, which_roi)]
        
        sdata = run_pca_on_subject(subject,epochs, fwd, cov, labels)
            ...
            if (self._mus_smooth_lst is None or self._sigmas_smooth_lst is None
                ...
                with numpy_num_threads(1):
                    _, mus_smooth, sigmas_smooth, sigmas_tnt_smooth = \
                            rts_smooth_fast(Y, self.A, fwd_roi_snsr, roi_cov_t, R, self.mu0,
    ...
    def fit(self, niter=100, tol=1e-6, A_roi_cov_niter=100, A_roi_cov_tol=1e-6, verbose=0,
           update_A=True, update_roi_cov=True, update_roi_cov_0=True, stationary_A=False,
        ...
        # set parameters for (A, roi_cov) optimization
        self._A_roi_cov_niter = A_roi_cov_niter
        self._A_roi_cov_tol = A_roi_cov_tol
        ...
        best_params = (self.A.copy(), self.roi_cov.copy(), self.mu0.copy(),
        ...
        if Atrue is not None:
            ...
            fig_A, ax_A = plt.subplots(num_roi, num_roi, sharex=True, sharey=True)
            plt.ion()
        ...
        for it in range(1, niter+1):
            ...
            self.m_step(update_A=update_A, update_roi_cov=update_roi_cov,
                        update_roi_cov_0=update_roi_cov_0, stationary_A=stationary_A,
            ...
            A, roi_cov, mu0, roi_cov_0, log_sigsq_lst = best_params
            self.A = A.copy()
            ...
            for s, sdata in enumerate(self.unpack_all_subject_data()):
                ...
                with numpy_num_threads(1):
                    loglik_subject, mus_smooth, _, _, St = \
                        rts_smooth(Y, self.A, fwd_roi_snsr, roi_cov_t, R, self.mu0, self.roi_cov_0,
    ...
    def m_step(self, update_A=True, update_roi_cov=True, update_roi_cov_0=True,
        stationary_A=False, diag_roi_cov=False, update_sigsq=True, tau=0.1, c1=1e-4,
        verbose=0):
        self._loglik = None
        if verbose > 0:
            print("  m-step")
        if update_roi_cov_0:
            ...
            if diag_roi_cov:
                self.roi_cov_0 = np.diag(np.diag(self.roi_cov_0))
        self.update_A_and_roi_cov(update_A=update_A, update_roi_cov=update_roi_cov,
                            stationary_A=stationary_A, diag_roi_cov=diag_roi_cov,
                            tau=tau, c1=c1, verbose=verbose)
        if update_sigsq:
            self.update_log_sigsq_lst(verbose=verbose)

    def update_A_and_roi_cov(self, update_A=True, update_roi_cov=True, stationary_A=False,
        diag_roi_cov=False, tau=0.1, c1=1e-4, verbose=0):

        if verbose > 1:
            print("    update A and roi_cov")
        ...
        while np.abs(obj_diff / obj) > self._A_roi_cov_tol:

            if inner_it > self._A_roi_cov_niter:
                break
    ...
    def log_likelihood(self):
        ...
        for s, sdata in enumerate(self.unpack_all_subject_data()):
            ...
            R = MEGLDS.R_(snsr_cov, fwd_src_snsr, sigsq_vals, which_roi)
            roi_cov_t = _ensure_ndim(self.roi_cov, self._timepts, 3)
            ll, _, _, _ = kalman_filter(Y, self.A, fwd_roi_snsr, roi_cov_t, R, self.mu0,
    ...
    def save(self, filename, **kwargs):
        savedict = { 'A' : self.A, 'roi_cov' : self.roi_cov, 'mu0' : self.mu0,
    ...
    def load(self, filename):
        ...
        param_names = ['A', 'roi_cov', 'mu0', 'roi_cov_0', 'log_sigsq_lst', 'lam0', 'lam1']
